...
// get text message
const get_message_text = (arg, done) => {
    try {
        stemp = document.querySelector('.msg-thread')
        stemp = stemp.querySelectorAll('.msg-s-event-listitem__body')
        result = []
        var i;
...
const create_csv = (array_list, header, n_line) => {
    ...
    for (i = 0; i < array_lenght; i++) {
        ...
        for (y = 0; y < array_list.length; y++) {
            ...
            try {
                // convert to string & replace , by comma
                stemp = String(stemp)
...
async function main() {
    ...
    for (i = 0; i < arg.AccountNumber; i++) {
        ...
        try {
            ...
            try {
                stemp = await tab.evaluate(get_message_text, arg)
                console.log(stemp)
    ...
    result_object = {
        "thread": thread_list, "time_seprator_list": time_seprator_list, "conterpart_list": conterpart_list, "message_count_list": message_count_list, "interlocutor": interlocutor, "message_text": message_text}
    ...
    header = ["thread", "interlocutor", "time_seprator_list", "conterpart_list", "message_count_list", "message_text"]
    array_list = [thread_list, interlocutor, time_seprator_list, conterpart_list, message_count_list, message_text]
